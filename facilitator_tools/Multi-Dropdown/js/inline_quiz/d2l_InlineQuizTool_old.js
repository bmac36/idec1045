/**
* 
* File : d2l_InlineQuizTool.js
* 
* Project : Vanilla - Inline Assessment Tool
* 
* Author : CJ Morrison & Justin Schlumkoski
* 
* Created : 01/13/2015
*
* Last Modified: 06/08/2015
* 
* Description : This is the engine that builds an inline assessment activity using a user-created data file.
*
*/
 ...
 * headerRef - Holds a created HTML element. String.
...
InlineQuizApp.HandleWindowAdjust = function(){ 
  ...
  if ($(".ILQ_ContinuousResponceText").length != 0 && $("#ILQ_NextButton").length != 0  ) 
  {
    ...
    if(w1 < InlineQuizApp.smallWidthTolerance) {
      ...
      //$("#ILQ_NextButton").css("margin", "2em auto 0px auto")
    } else {
      ...
      //$("#ILQ_NextButton").css("margin", "2em 0px 0px 0px")
    }
  }
...
InlineQuizApp.GenerateFullReport = function() {
  ...
  var ILQ_ContinuousResponceHeader = document.createElement("h2");
  ILQ_ContinuousResponceHeader.setAttribute('id', 'ILQ_ContinuousResponceHeader');
  ILQ_ContinuousResponceHeader.innerHTML = 'Results: ';
  ILQ_fullReport.appendChild(ILQ_ContinuousResponceHeader);
...
InlineQuizApp.GenerateMiniReport = function(questionIndex, includeQuestion) {
  ...
  if(includeQuestion){
    var ILQ_miniReportRecap = document.createElement("h3");
  ...
  if(crta.length > 1)
  {
    cont += "<h4>Correct Answers:</h4><ul>"
  } else {
    cont += "<h4>Correct Answer:</h4><ul>"
  }
    ...
    if(ca.length == 0){
      ...
      for (var i = ca.length - 1; i >= 0; i--) {
        if( aa[ca[i]].feedBack == void 0 || aa[ca[i]].feedBack == "none" || aa[ca[i]].feedBack == null )
        {
          ...
          cont += "<li>"+aa[ca[i]].answerText+"<br><strong>Feedback: "+aa[ca[i]].feedBack+"</strong></li>"
        }
      }
    }
  }
...
InlineQuizApp.BuildResponceText = function(containerRef, questionIndex) {
    var ILQ_RecapResponceText = document.createElement("h4");
...
InlineQuizApp.DefineContainer = function() {
  ...
  var ILQ_container = document.createElement("section");
  ...
  if(!InlineQuizApp.QuizData.General.hideTitle)
  {
    ...
    InlineQuizApp.headerRef.innerHTML = "<h1>"+InlineQuizApp.QuizData.General.QuizName+"</h1>";
  }
...
InlineQuizApp.MakeFullBaseButton = function(onOK, lable, options){;
  ...
  if(!options.disabled){
    ...
    ILQ_FullBaseButtonContainer.onmousedown = function(e){
...
InlineQuizApp.MakeBaseButtonSet = function(leftBtnData, RightButtonData){
  ...
  if(leftBtnData != null)
  {
    ...
    if(!leftBtnData.options.disabled){
      ...
      ILQ_LeftBaseButtonContainer.onmousedown = function(e){
        $(this).addClass("active")
      }
  ...
  if(RightButtonData != null)
  {
    ...
    if(!RightButtonData.options.disabled){
      ...
      ILQ_RightBaseButtonContainer.onmousedown = function(e){
        $(this).addClass("active")
      }
...
InlineQuizApp.MakeGenericButton = function(onOK, lable, options){ // next button
  ...
  if(!options.disabled){
    ...
    ILQ_GenericButtonContainer.onmousedown = function(e){
      $(this).addClass("active")
    }
...
InlineQuizApp.SetTextSlide = function(data, onOK, options){	
  ...
  {
    options.buttonLable = "Start Activity";
  } 
  ...
  if(InlineQuizApp.isHint == true){
    ILQ_Description.innerHTML = "<h2 class=\"ILQ_hintHeader\">Hint: Question " + (InlineQuizApp.currentQuestion + 1) + "</h2>" + data;
  }
  else {
  ...
  var ILQ_buttonSet = document.createElement("div");
  ...
  if(onOK !== void 0 && onOK !== "none"){
    ILQ_buttonSet.appendChild(InlineQuizApp.MakeFullBaseButton(onOK, options.buttonLable));
  } else {
...
InlineQuizApp.setQuestionSlide = function(data, options){	
  ...
  var ILQ_questionHeader = document.createElement("h2");
  ...
  if(InlineQuizApp.QuizData.General.allowPrevious){
    ...
    ILQ_buttonSet.appendChild(InlineQuizApp.MakeFullBaseButton(InlineQuizApp.RequestNextQuestion, options.buttonLable, {
      disabled:!InlineQuizApp.QuizData.General.allowNone,
      id:"ILQ_quizNextBtn"
    }));
  }
...
InlineQuizApp.ShowHint = function() {
    ...
    }, 
    {
        ...
        buttonLable: "Back to Question"
    })
}

/**
 * Function: RepopulateQuestion
 * 
...
InlineQuizApp.RepopulateQuestion = function(){
  if(InlineQuizApp.QuizData.Questions[InlineQuizApp.currentQuestion].questionType == "All That Apply" || InlineQuizApp.QuizData.Questions[InlineQuizApp.currentQuestion].questionType == "Multiple Choice"){
    for (var i = 0; i < InlineQuizApp.QuizData.Questions[InlineQuizApp.currentQuestion].ChoosenAnswers.length; i++){
      temp = document.getElementById("AnswerSlot_" + InlineQuizApp.QuizData.Questions[InlineQuizApp.currentQuestion].ChoosenAnswers[i]);
    ...
    for (var i = 0; i < InlineQuizApp.savedText[InlineQuizApp.currentQuestion].length; i++){
...
InlineQuizApp.HandleAnswerSelection = function(e) {
  if (e == "repop"){
    ...
    if (selectedSlot.className.indexOf("ILQ_AnswerSlot") == -1 ){ // in case we click the label somehow
      ...
      //ADDED TO ALLOW FOR WRAPPING ANSWERTEXT INSIDE OF TAGS
...
InlineQuizApp.AssessFeedback = function() {
  ...
  var ILQ_ContinuousResponceText = document.createElement("p");
  ...
  if (InlineQuizApp.QuizData.General.forceCorrect == true){
    ...
    ILQ_ContinuousResponceCont.appendChild(ILQ_ContinuousResponceHeader);
